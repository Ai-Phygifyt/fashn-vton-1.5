...
from datasets.preprocess import cache_paths, is_cached
...
class SareeVTONDataset(Dataset):
    ...
    def __init__(
        self, cfg: Config, csv_path: str | Path, split: str = "train", logger: Optional[logging.Logger] = None
    ):
        self.cfg = cfg
        self.split = split
        ...
        self.n_failures = 0
        self.logger.info(
            f"{split}: {len(self.df)} samples  ({cfg.data.height}x{cfg.data.width}, "
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        raise RuntimeError(
            f"5 consecutive sample load failures starting at index {idx}; the cache is likely corrupt."
        )
...
def build_dataloader(
    cfg: Config, split: str = "train", logger=None, shuffle: Optional[bool] = None
) -> DataLoader:
