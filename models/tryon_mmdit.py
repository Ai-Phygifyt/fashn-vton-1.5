...
class TryOnModel(nn.Module):
    def __init__(
        ...
        self.y_embedder = (
            nn.Embedding(n_classes + 1, hidden_size) if n_classes > 0 else None
        )  # +1 for null class
        ...
        if use_patch_mixer:
            self.x_patch_mixer = nn.ModuleList(
                [
                    SingleStreamBlock(hidden_size, n_heads, mlp_ratio=mlp_ratio)
        ...
        # Gradient checkpointing (training only; OFF by default so inference is unchanged).
        self.gradient_checkpointing = False

    def set_gradient_checkpointing(self, enabled: bool = True) -> int:
        """
        Enable/disable activation checkpointing over the transformer block stacks.

        Trades ~30-40% extra compute for a large activation-memory saving, which is what
        makes LoRA fine-tuning fit on a 4 GB card. Has no effect under inference_mode /
        no_grad, so leaving it on costs nothing at inference time.

        Returns the number of block stacks affected.
        """
        self.gradient_checkpointing = bool(enabled)
        return 3 if self.use_patch_mixer else 2

    def _maybe_checkpoint(self, fn, *args):
        """Apply torch.utils.checkpoint only when it can actually do anything."""
    ...
    def forward_for_cfg(self, *args, **kwargs):
        ...
        duplicated_args = [
            torch.cat([arg, arg], dim=0) if isinstance(arg, torch.Tensor) else arg for arg in args
        ]
        duplicated_kwargs = {
    ...
    def forward(
        ...
        if self.use_patch_mixer:
            for block in self.x_patch_mixer:
                img = self._maybe_checkpoint(block, img, vec, x_pe)
        ...
        for block in self.double_blocks:
            img, txt = self._maybe_checkpoint(block, img, txt, vec, pe)

        img = torch.cat((txt, img), 1)
        for block in self.single_blocks:
            img = self._maybe_checkpoint(block, img, vec, pe)
        img = img[:, txt.shape[1] :, ...]
