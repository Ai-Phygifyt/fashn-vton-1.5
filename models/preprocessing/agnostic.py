...
def _create_hybrid_contour_bounded_mask(
    ...
    distance_from_contour = cv2.distanceTransform((~contour_mask).astype(np.uint8), cv2.DIST_L2, 5)
...
def create_clothing_agnostic_image(
    ...
    ca_mask = asymmetric_dilate_mask(
        ca_mask, right=scaled_right, left=scaled_left, up=scaled_up, down=scaled_down
    )
