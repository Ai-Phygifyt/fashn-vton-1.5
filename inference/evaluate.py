...
(CONTEXT.md §13.3).
    ...
    python -m inference.evaluate --subset eval/eval_subset.csv --out outputs/eval_base

    # a fine-tuned LoRA checkpoint
    python -m inference.evaluate --subset eval/eval_subset.csv \
...
from inference.metrics import build as build_metrics
from inference.metrics import describe


def load_pipeline(
    weights_dir: str, lora_ckpt: Optional[str], device: Optional[str], aux_device: str = "cpu", logger=None
):
    """
    ...
    if lora_ckpt:
        ...
        if logger:
            logger.info(
                f"applied + merged {n} LoRA modules from {lora_ckpt} "
...
def compare(a: str, b: str, out: Optional[str] = None) -> pd.DataFrame:
    ...
    from inference.metrics import _REGISTRY
...
def make_boards(
    ...
    mb = pd.read_csv(b_dir / "metrics.csv", dtype={"id": str}).set_index("id") if b_dir else None
    ...
    for sid in ids:
        panels = [
            (fit(Image.open(a_dir / "samples" / sid / "person.jpg").convert("RGB")), "Person (input / GT)"),
        ...
        if b_dir and (b_dir / "samples" / sid / "output.png").exists():
            panels.append(
                (
                    fit(Image.open(b_dir / "samples" / sid / "output.png").convert("RGB")),
                    f"Output — {label_b}",
                )
            )
        ...
        if meta is not None:
            bits = [
                str(meta.get(k, "")) for k in ("source", "fabric_family", "target_type") if k in ma.columns
            ]
...
def main():
    ...
    ap.add_argument(
        "--masked", action="store_true", help="segmentation_free=False — the honest try-on measurement"
    )
