...
from training import memory as mem
from training.config import LORA_PRESETS, Config, LoRAConfig
...
def model_kwargs(bs: int = 1):
    b = dummy_batch(bs)
    return dict(
        ca_images=b["ca_image"],
        garment_images=b["garment"],
        person_poses=b["person_pose"],
        garment_poses=b["garment_pose"],
...
class TestConfig:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ["data.height=100"],
            ["lora.rank=0"],
            ["train.cond_dropout_prob=2"],
            ["memory.precision=int4"],
            ["train.timestep_sampling=magic"],
            ["lora.preset=nope"],
        ],
    )
...
class TestLoss:
    ...
    def test_loss_positive_and_finite(self):
        ...
        loss = losses.flow_matching_loss(a, b, t, cfg)
...
class TestGradients:
    ...
    def test_optimizer_updates_lora_only(self):
        ...
        L.mark_only_lora_trainable(m)
        m.train()
        from training.engine import build_optimizer
...
class TestScheduler:
    def _lrs(self, cfg, total=100):
        ...
        L.mark_only_lora_trainable(m)
        from training.engine import build_optimizer, build_scheduler
...
class TestDataset:
    def _make_cache(self, root: Path, ids, h=H, w=W):
        from datasets.preprocess import SUBDIRS
        ...
        for sid in ids:
            Image.fromarray(np.random.randint(0, 255, (h, w, 3), dtype=np.uint8)).save(
                cache / "person" / f"{sid}.jpg"
            )
            Image.fromarray(np.random.randint(0, 255, (h, w, 3), dtype=np.uint8)).save(
                cache / "garment" / f"{sid}.jpg"
            )
            ...
            Image.fromarray(np.random.randint(0, 18, (h, w), dtype=np.uint8)).save(
                cache / "parse" / f"{sid}.png"
            )
        return cache
    ...
    def test_dataset_shapes_and_range(self, tmp_path):
        from datasets.dataset import SareeVTONDataset
...
class TestCleaning:
    def test_label_ids_come_from_the_package(self):
        ...
        from datasets import clean as C
    ...
    def test_blouse_render_detector(self, row, expected):
        from datasets.clean import is_blouse_render
...
class TestEvaluationRegistry:
    def test_registry_has_pixel_and_saree_metrics(self):
        from inference import available
