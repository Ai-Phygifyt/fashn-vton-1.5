...
Training entrypoint for saree LoRA fine-tuning.
    ...
    python train.py --config configs/default.yaml --set optim.lr=5e-5 train.epochs=20
...
def parse_args(argv=None):
    ...
    ap.add_argument(
        "--dry-run", action="store_true", help="build everything and run a couple of steps, then exit"
    )
...
def main(argv=None) -> int:
    ...
    from training.engine import Trainer, build_model, build_optimizer, build_scheduler
    from training.logging_utils import TrainLogger, setup_run_logger
    ...
    boot.info(
        f"train batches {len(train_loader)} | steps/epoch {steps_per_epoch} | "
    ...
    (run_dir / "model_info.json").write_text(
        json.dumps({k: v for k, v in info.items() if k != "modules"}, indent=2, default=str)
    )

    trainer = Trainer(
        cfg, model, optimizer, scheduler, device, dtype, tlogger, ckpt, train_loader, val_loader
    )
    if resumed:
        ...
        for epoch in range(start_epoch, cfg.train.epochs):
            ...
            if (epoch + 1) % max(1, cfg.checkpoint.save_every_epochs) == 0 or is_best:
                ckpt.save(
                    ...
                    step=trainer.step,
                    metrics=metrics,
                    tag=f"epoch{epoch + 1:04d}.pt",
                    is_best=is_best,
                )

            tlogger.info(
                f"epoch {epoch + 1} done in {stats['epoch_seconds'] / 60:.1f} min "
