...
class LoRALinear(nn.Module):
    ...
    def __init__(
        self, base: nn.Linear, rank: int, alpha: int, dropout: float = 0.0, init_scale: float = 0.01
    ):
        super().__init__()
        if rank <= 0:
        ...
        for k, p in own.items():
            if k in sd:
                if sd[k].shape != p.shape:
                    raise RuntimeError(
                        f"shape mismatch for {k}: ckpt {tuple(sd[k].shape)} vs model {tuple(p.shape)}"
                    )
...
def summarise(model: nn.Module, info: Optional[Dict] = None) -> str:
    ...
    lines = [
        f"LoRA: {info['n_modules']} modules adapted (rank {info['rank']}, alpha {info['alpha']})"
        if info
        else "LoRA",
