...
def build_clean_split(cfg: Config, stats: pd.DataFrame, logger=None) -> Dict[str, pd.DataFrame]:
    ...
    if c.reject_blouse_renders:
        step("reject blouse-piece render",
    ...
    if c.enable_collage_filter:
        ...
        step(f"reject collage (comp>{c.max_foreground_components})",
...
def main():
    ...
    if missing:
        ...
        for i, r in enumerate(todo.itertuples(), 1):
            ...
            try:
                ...
                rec.update(colorfulness=0.0, white_frac=1.0, n_components=0,
                           face_frac=1.0, limb_frac=1.0, probe_ok=False)
                logger.warning(f"probe failed for {r.id}: {type(e).__name__}: {e}")
            rows.append(rec)
            if i % 250 == 0:
                el = time.time() - t0
                logger.info(f"  {i}/{len(todo)}  {el/i:.3f}s/img  eta {(len(todo)-i)*el/i/60:.1f} min")
    ...
    logger.info(f"\nWrote {od}/clean_train.csv ({len(out['train'])}) and "
                f"clean_validation.csv ({len(out['val'])})")
