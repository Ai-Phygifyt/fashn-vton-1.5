...
Usage:
    python -m fashn_vton.training.data.preprocess --config configs/default.yaml --split train
"""
...
class Preprocessor:
    ...
    def process(self, person_path: Path, garment_path: Path, garment_is_flat: bool = True) -> Dict[str, np.ndarray]:
...
def run(cfg: Config, csv_path: Path, cache_dir: Path, device: str, logger,
    ...
    logger.info(f"{csv_path.name}: {len(df)} samples, {len(todo)} to process "
                f"({len(df)-len(todo)} already cached)")
    if len(todo) == 0:
    ...
    for i, r in enumerate(todo.itertuples(), 1):
        ...
        if i % 100 == 0:
            el = time.time() - t0
            logger.info(f"  {i}/{len(todo)}  {el/i:.2f}s/sample  eta {(len(todo)-i)*el/i/60:.1f} min")

    return {"processed": len(todo) - len(failed), "failed": len(failed),
            "total": len(df), "failures": failed}
...
def main():
    ...
    for name, path in splits:
        if not path.exists():
            logger.error(f"{path} not found — run `python -m fashn_vton.training.data.clean` first")
            continue
