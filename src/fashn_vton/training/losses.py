...
def sample_timesteps(batch_size: int, cfg, device: torch.device,
...
def build_flow_batch(x1: torch.Tensor, cfg, generator: Optional[torch.Generator] = None
...
def flow_matching_loss(v_pred: torch.Tensor, v_target: torch.Tensor, t: torch.Tensor,
                       cfg) -> torch.Tensor:
    """
...
def make_cond_dropout_mask(batch_size: int, prob: float, device: torch.device,
                           generator: Optional[torch.Generator] = None) -> torch.Tensor:
    """
...
def compute_loss(model, batch: Dict[str, torch.Tensor], cfg,
