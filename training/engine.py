...
# ------------------------------------------------------------------------ model
def build_model(
    cfg: Config, device: torch.device, dtype: torch.dtype, logger=None
    ...
    log(
        f"parameters: {stats['trainable']:,} trainable / {stats['total']:,} total "
        f"({stats['trainable_pct']:.3f}%)"
    )
    log(
        f"estimated static VRAM: {est['static_total_gb']:.2f} GB "
...
def build_optimizer(cfg: Config, model, logger=None):
    ...
    kw = dict(
        lr=cfg.optim.lr, betas=tuple(cfg.optim.betas), eps=cfg.optim.eps, weight_decay=cfg.optim.weight_decay
    )
    ...
    if cfg.optim.optimizer == "adamw8bit":
        try:
            ...
            if logger:
                logger.warning(
                    f"AdamW8bit unavailable ({type(e).__name__}: {e}); falling back to torch AdamW"
                )
...
class Trainer:
    ...
    def train_epoch(self, epoch: int, total_steps: int) -> Dict[str, float]:
        ...
        for i, raw in enumerate(self.train_loader):
            ...
            if self.cfg.checkpoint.save_every_steps and self.step % self.cfg.checkpoint.save_every_steps == 0:
                self.ckpt.save(
                    self.model,
                    self.opt,
                    self.sched,
