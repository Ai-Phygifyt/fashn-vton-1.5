    ...
    def save(
        ...
        *,
        ...
        if is_best:
            ...
            self._log(
                f"new best ({self.cfg.checkpoint.monitor}={metrics.get(self.cfg.checkpoint.monitor)}) "
                f"-> {BEST}"
            )
        return target
    ...
    def load(
        ...
        if optimizer is not None and ck.get("optimizer"):
            try:
                ...
                self._log(
                    f"WARNING: optimizer state not restored ({type(e).__name__}: {e}); "
        ...
        self._log(
            f"resumed from {path.name}: epoch {ck['epoch']}, step {ck['step']}"
            + (
